...
import { setDisableBtn, setFetching, setEmailTemplateModal } from '@/store/Slices/leadSlice/manageLeadSlice';
...
const EmailTemplateModal = () => {
    const { isBtnDisabled, singleData, isFetching, emailTemplateModal } = useSelector((state: IRootState) => state.lead);
    ...
    const onSubmitPolicy = async () => {
        ...
        try {
            ...
            dispatch(setEmailTemplateModal({ open: false }));
        } catch (error: any) {
    ...
    const getAllTemplates = async () => {
        const templateList: GetMethodResponseType = await new ApiClient().get('email-template/list');
    ...
    return (
        <Modal
            ...
            onClose={() => dispatch(setEmailTemplateModal({ open: false }))}
            size="medium"
            onDiscard={() => dispatch(setEmailTemplateModal({ open: false }))}
            ...
            content={
                isFetching ? (
                    ...
                    <div className="text-center text-xl">
                        <Select placeholder="Select Email Template" options={templateDropdown} isSearchable={false} onChange={(e: any) => setTemplateId(e.value)} />
                    </div>
                )
            }
        />
