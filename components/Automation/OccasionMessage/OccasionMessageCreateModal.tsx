...
import { setCreateModal, setDisableBtn, setFetching } from '@/store/Slices/automationSlice/occasionMessageSlice';
...
const OccasionMessageCreateModal = () => {
    const { createModal, isBtnDisabled, isFetching, sourceList } = useSelector((state: IRootState) => state.occasionMessage);
    ...
    const { values, handleChange, submitForm, handleSubmit, handleBlur, resetForm, setFieldValue } = useFormik({
        ...
        onSubmit: async (value, action) => {
            ...
            try {
                ...
                await new ApiClient().post('occasionMessage', { name: value.name });
...
export default memo(OccasionMessageCreateModal);
