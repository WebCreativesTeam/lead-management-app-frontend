...
const SourcePage = () => {
        ...
        onSubmit: async (value, action) => {
            try {
                if (editModal) {
                    ...
                    await axios.patch('http://15.206.70.64:3030/sources/' + singleSource.id, editSourceObj);
                    setDisableBtn(false);
                    action.resetForm();
                    setEditModal(false);
                } else if (createModal) {
                    ...
                    await axios.post('http://15.206.70.64:3030/sources', createSourceObj);
                    setDisableBtn(false);
    ...
    const getSourceList = async () => {
        try {
            const res = await axios.get('http://15.206.70.64:3030/sources');
    ...
    const onDeleteSource = async () => {
        try {
            setDisableBtn(true);
            await axios.delete('http://15.206.70.64:3030/sources/' + singleDeleteSource);
            setDisableBtn(false);
            setDeleteModal(false);
        } catch (error: any) {
            setDisableBtn(true);
    ...
    return (
        <div>
            ...
            <div className="table-responsive my-5 ">
                <h2 className="my-4 text-center text-2xl font-bold dark:text-white md:text-5xl">Source List</h2>
                <table>
                    <thead>
                        <tr>
                            <th>#</th>
                    ...
                    <tbody>
                        {searchedData?.map((item: SourceDataType, i) => {
                            return (
                                <tr key={item.id}>
                                    <td>{i + 1}</td>
                                    <td>
                                    ...
                                    <td className="flex gap-2 border-b border-[#ebedf2] p-3 text-center dark:border-[#191e3a]">
                                        <Tippy content="View">
                                            <button type="button" onClick={() => handleViewSource(item.id)}>
                                        ...
                                        <Tippy content="Edit">
                                            <button type="button" onClick={() => handleEditSource(item.id)}>
                                        ...
                                        <Tippy content="Delete">
                                            <button type="button" onClick={() => handleDeleteSource(item.id)}>
