...
const BoxedPolicy = () => {
        ...
        onSubmit: async (value, action) => {
            try {
                if (editModal) {
                    ...
                    await axios.patch('http://13.233.50.11:3030/policies/' + singlePolicy.id, editPolicyObj);
                    setDisableBtn(false);
                    action.resetForm();
                    setEditModal(false);
                    setPermissionArr([]);
                    ...
                    await axios.post('http://13.233.50.11:3030/policies/', createPolicyObj);
    ...
    const getPolicyList = async () => {
        try {
            const res = await axios.get('http://13.233.50.11:3030/policies');
    ...
    const onDeletePolicy = async () => {
        try {
            setDisableBtn(true);
            await axios.delete('http://13.233.50.11:3030/policies/' + singleDeletePolicy);
            setDisableBtn(false);
            setDeleteModal(false);
        } catch (error: any) {
            setDisableBtn(true);
        ...
        <div>
            ...
            <div className="my-6 flex flex-col gap-5 sm:flex-row ">
                ...
                <div className="relative  flex-1">
                    <input type="text" placeholder="Find A Policy" className="form-input py-3 ltr:pr-[100px] rtl:pl-[100px]" />
            ...
            <div className="table-responsive my-5 ">
                ...
                <table>
                    ...
                    <tbody>
                        {data.map((item: PolicyDataType, i) => {
                            return (
                                <tr key={item.id}>
                                    <td>{i + 1}</td>
                                    <td>
                                            ...
                                            <form className="space-y-5" onSubmit={handleSubmit}>
                                                ...
                                                <div className="mt-8 flex items-center justify-end">
                                                    ...
                                                    <input
                                                        ...
                                                        className="btn btn-primary cursor-pointer ltr:ml-4 rtl:mr-4"
                                                        value="Create Policy"
                                                        disabled={values.policyName && values.policyDescription && !disableBtn ? false : true}
                                                    />
                                                </div>
                                            </form>
                                        </div>
