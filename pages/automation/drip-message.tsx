...
import { GetMethodResponseType, IDripMessage, LeadStatusSecondaryEndpoint, SourceDataType } from '@/utils/Types';
...
import {
    ...
    setViewModal,
} from '@/store/Slices/automationSlice/dripMessageSlice';
import DripMessageViewModal from '@/components/Automation/DripMessage/DripMessageViewModal';
import DripMessageCreateModal from '@/components/Automation/DripMessage/DripMessageCreateModal';
import DripMessageEditModal from '@/components/Automation/DripMessage/DripMessageEditModal';
import DripMessageDeleteModal from '@/components/Automation/DripMessage/DripMessageDeleteModal';
...
const DripMessage = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Track Leads | DripMessages'));
    });
    const { data, isFetching, isAbleToCreate, isAbleToDelete, isAbleToUpdate, totalRecords } = useSelector((state: IRootState) => state.dripMessage);
    ...
    const [recordsData, setRecordsData] = useState<IDripMessage[]>([] as IDripMessage[]);
    ...
    //get all DripMessage after page render
    useEffect(() => {
        getDripMessageList();
    ...
    //get all DripMessage list
    const getDripMessageList = async () => {
        setLoading(true);
        // const res: GetMethodResponseType = await new ApiClient().get(`schedule-message?limit=${pageSize}&page=${page}&search=${searchQuery}`);
        // const DripMessage: IDripMessage[] = res?.data;
        // if (typeof DripMessage === 'undefined') {
        //     dispatch(getAllDripMessages([] as IDripMessage[]));
        //     return;
        // }
        dispatch(getAllDripMessages(scheduleMessagesRawData));
        dispatch(setDripMessageDataLength(scheduleMessagesRawData.length));
        setLoading(false);
    };
    ...
    const getAllSourceList = async () => {
        ...
        if (typeof source === 'undefined') {
            dispatch(getAllSourceForDripMessage([] as SourceDataType[]));
            return;
        }
        dispatch(getAllSourceForDripMessage(source));
    };
    ...
    const getLeadStatus = async () => {
        ...
        if (typeof status === 'undefined') {
            dispatch(getAllLeadStatusForDripMessage([] as LeadStatusSecondaryEndpoint[]));
            return;
        }
        dispatch(getAllLeadStatusForDripMessage(status));
    };

    return (
        <div>
            <PageHeadingSection description="Drip message using template and for different platforms." heading="Drip Message" />
            <div className="my-6 flex flex-col gap-5 sm:flex-row ">
                ...
                <div className="relative  flex-1">
                    <input
                        type="text"
                        placeholder="Find A DripMessage"
            ...
            {/* edit modal */}
            <DripMessageEditModal />

            {/* view modal */}
            <DripMessageViewModal />

            {/* delete modal */}
            <DripMessageDeleteModal />

            {/* create modal */}
            <DripMessageCreateModal />
        </div>
    );
};
